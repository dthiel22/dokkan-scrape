...
URL2 = ("https://dbz-dokkanbattle.fandom.com/wiki/Planet-Crushing_Blow_Cooler_(Final_Form)?action=edit")
print(URL2)
...
while myline:
    myline = test.readline()
    if '"ID":' in myline:
        charIDAPI.append(myline.replace('"ID"',"").replace(': ','"',1).replace('\n','": {\n'))
    if '"name1": ' in myline:
...
if charPsName[0] == [""]:
    results = []
if "None" in charLinkSkills[0] or charLinkSkills[0] == [""]:
    results = []
if charCategories == [""]:
    results = []
if '"N"' in charRarity:
