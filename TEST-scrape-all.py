...
# loops through pages of cards x < 2600 (x needs to be 1) ========
x = 201
while x < 2600:
    x += 100
    print()
    ...
    while i <= numOfCharacters-2:
        ...
        URL2 = (
            f"https://dbz-dokkanbattle.fandom.com{characterSelectEl}?action=edit")
        print(URL2)
        ...
        while myline:
            myline = test.readline()
            if '|name1": ' in myline:
                charName1.append(myline.replace('|', '"').replace("name1", "title").replace(
                    ': ', ': "', 1).replace('\n', '",\n'))
            if '|name2":' in myline:
                charName2.append(myline.replace('|', '"').replace("name2", "name").replace(
                    ': ', ': "', 1).replace('\n', '",\n'))
            if '|rarity":' in myline:
                charRarity.append(myline.replace('|', '"').replace(
                    ': ', ': "', 1).replace('\n', '",\n'))
            if '|type":' in myline:
                charType.append(myline.replace('|', '"').replace(
                    ': ', ': "', 1).replace('\n', '",\n'))
            if '|cost":' in myline:
                charCost.append(myline.replace('|', '"').replace(
                    ': ', ': "', 1).replace('\n', '",\n'))
            if '|ID":' in myline:
            ...
            if '|LS description Z":' in myline:
                charLSEza.append(myline.replace('|', '"').replace("LS description Z", "ls_description_eza").replace(
                    ': ', ': "', 1).replace('\n', '",\n'))
            if '|SA type":' in myline:
                charSaType.append(myline.replace('|', '"').replace('SA type', 'sa_type').replace(
                    ': ', ': "', 1).replace('\n', '",\n'))
            ...
            if '|SA type Active":' in myline:
                charASType.append(myline.replace('|', '"').replace('SA type Active', 'sa_type_active').replace(
                    ': ', ': "', 1).replace('\n', '",\n'))
