...
URL2 = ("https://dbz-dokkanbattle.fandom.com/wiki/Calm_and_Collected_Hero_Gamma_1?action=edit")
print(URL2)
...
while myline:
    myline = test.readline()
    if '|ID":' in myline:
        charIDAPI.append(myline.replace('ID"',"ID").replace("|","").replace(': ','',1).replace('\n','= {\n'))
    if '|name1": ' in myline:
        charName1 = myline.replace("|","").replace('": ',': "',1).replace('\n','",\n')
    if '|name2":' in myline:
        charName2 = myline.replace("|","").replace('": ',': "',1).replace('\n','",\n')
    if '|rarity":' in myline:
        charRarity = myline.replace("|","").replace('": ',': "',1).replace('\n','",\n')
    if '|type":' in myline:
        charType = myline.replace("|","").replace('": ',': "',1).replace('\n','",\n')
    if '|ID":' in myline:
        charID.append(myline.replace("|","").replace('": ',': "',1).replace('\n','",\n'))
    if '|LS description":' in myline:
        charLS = myline.replace("LS description","LS").replace("|","").replace('": ',': "',1).replace('\n','",\n')
    if '|SA name":' in myline:
        charSaName.append(myline.replace("SA name","saName").replace("|","").replace('": ',': "',1).replace('\n','",\n'))
    if '|SA description":' in myline:
        charSaDesc.append(myline.replace("SA description","saDesc").replace("|","").replace('": ',': "',1).replace('\n','",\n'))
    if '|UltraSA name":' in myline:
        charUltraName.append(myline.replace("UltraSA name","usaName").replace("|","").replace('": ',': "',1).replace('\n','",\n'))
    if '|UltraSA description":' in myline:
        charUltraDesc.append(myline.replace("UltraSA description","usaDesc").replace("|","").replace('": ',': "',1).replace('\n','",\n'))
    if '|PS name":' in myline:
        charPsName.append(myline.replace("PS name","psName").replace("|","").replace('": ',': "',1).replace('\n','",\n'))
    if '|PS description":' in myline:
        charPsDesc.append(myline.replace("PS description","psDesc").replace("|","").replace('": ',': "',1).replace('\n','",\n'))
    if '|Active skill name":' in myline:
        charASName = myline.replace("Active skill name","asName").replace("|","").replace('": ',': "',1).replace('\n','",\n')
    if '|Active skill":' in myline:
        charAS = myline.replace("Active skill","aSkill").replace("|","").replace('": ',': "',1).replace('\n','",\n')
    if '|Active skill condition":' in myline:
        charASCond = myline.replace("Active skill condition","asDesc").replace("|","").replace('": ',': "',1).replace('\n','",\n')
    if '|Transform type":' in myline:
        charTransformType = myline.replace("Transform type","tType").replace("|","").replace('": ',': "',1).replace('\n','",\n')
    if '|Transform condition":' in myline:
        charTransformCond.append(myline.replace("Transform condition","tCond").replace("|","").replace('": ',': "',1).replace('\n','",\n'))
    if '|Link skill":' in myline:
        charLinkSkills.append(myline.replace("Link skill","linkSkills").replace("|","").replace(",",'","').replace('" ','"').replace('": ',': ["',1).replace('\n','"],\n'))
    if '|Category":' in myline:
        charCategories = myline.replace("Category","Categories").replace("|","").replace(",",'","').replace('" ','"').replace('": ',': ["',1).replace('\n','"]},\n')
...
results = [extraSpace1, charIDAPI[0], charSourceLink, charName1, charName2, charRarity, charType, charID[0], charLS, charSaName[0], charSaDesc[0], charUltraName[0], charUltraDesc[0], charPsName[0], charPsDesc[0], charASName, charAS, charASCond, charTransformType, charTransformCond[0], charLinkSkills[0], charCategories, extraSpace2]

if "None" in charLinkSkills[0]:
    results = []
if charCategories == []:
    results = []
if '"R"' in charRarity:
...
with open('out.js', 'a', encoding="utf-8") as output:
