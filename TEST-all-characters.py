...
while x < 2600:
    ...
    while i <= numOfCharacters-2:
        ...
        #TODO: LINK SKILL PORTION
        linkArr = linkSeed.split(',')
        linkResults = []
        for singleLink in linkArr:
            linkResults.append('{\n')
            linkResults.append('CharacterId: '+idSeed.replace('id: ','').replace('\n', '')+'\n')
            linkResults.append('LinkName: '+'"'+singleLink.replace('\n', '')+'"')
            linkResults.append('\n},\n')
        ...
        with open('CharacterLink.js', 'a', encoding="utf-8") as output:
